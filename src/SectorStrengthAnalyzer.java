...
/**
 * Loads sector/industry/basic_industry taxonomy and computes sector-level RS rankings.
 ...
 * Format: nse_ticker,sector,industry,basic_industry,notes
 */
public class SectorStrengthAnalyzer {
        ...
        try {
            ...
            for (String line : lines) {
                ...
                if (parts.length >= 3) {
                    ...
                    String basicIndustry = parts.length >= 4 ? parts[3].trim() : "";
                    if (!ticker.isEmpty() && !sector.isEmpty()) {
                        ...
                        if (!basicIndustry.isEmpty()) {
                            symbolToBasicIndustry.put(ticker, basicIndustry);
                            symbolToBasicIndustry.put(ticker + ".NS", basicIndustry);
                        }
                    }
                }
            }
    ...
    public String getBasicIndustry(String symbol) {
        return symbolToBasicIndustry.getOrDefault(symbol,
               symbolToIndustry.getOrDefault(symbol, ""));
    }
    ...
    public Map<String, Double> computeBasicIndustryStrength(Map<String, RelativeStrengthCalculator.RSProfile> rsProfiles) {
        Map<String, List<Double>> biScores = new HashMap<>();
        for (Map.Entry<String, RelativeStrengthCalculator.RSProfile> entry : rsProfiles.entrySet()) {
            String bi = symbolToBasicIndustry.get(entry.getKey());
            if (bi == null || bi.isEmpty()) {
                bi = symbolToIndustry.get(entry.getKey());
            }
            if (bi == null || bi.isEmpty()) continue;
            biScores.computeIfAbsent(bi, k -> new ArrayList<>()).add(entry.getValue().percentileRank);
        }
        Map<String, Double> result = new HashMap<>();
        for (Map.Entry<String, List<Double>> entry : biScores.entrySet()) {
            List<Double> scores = entry.getValue();
            double avg = scores.stream().mapToDouble(d -> d).average().orElse(50.0);
            result.put(entry.getKey(), avg);
        }
