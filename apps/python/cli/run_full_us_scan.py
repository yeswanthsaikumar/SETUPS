...
def save_html(rows: list[dict], path: Path, meta: dict):
    ...
    for r in rows:
        ...
        rows_html += (
            ...
            f"<td>{_disp(r.get('entryTriggerCondition'))}</td>"
            f"<td>{_disp(r.get('triggerEarningsGrowth'))}</td>"
            f"<td>{_disp(r.get('triggerDebtReduction'))}</td>"
            f"<td>{_disp(r.get('triggerMacroTailwind'))}</td>"
            f"<td>{_disp(r.get('triggerMarketTailwind'))}</td>"
  ...
  <div class="col-legend">
    ...
    <div class="col-legend-body" id="colLegendBody">
      <dl>
        ...
        <dt>Entry Trigger</dt><dd>The exact confirmation needed before acting, such as breakout close + healthy volume</dd>
        <dt>Earnings Trigger</dt><dd>Segregated fundamental catalyst bucket summarizing earnings/revenue acceleration</dd>
        <dt>Debt Trigger</dt><dd>Segregated balance-sheet catalyst bucket summarizing debt reduction or debt risk</dd>
        <dt>Macro Tailwind</dt><dd>Macro environment support bucket derived from the current regime context</dd>
        <dt>Market Tailwind</dt><dd>Market/relative-strength support bucket derived from weekly agreement and RS leadership</dd>
