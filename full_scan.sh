...
if [ "$SKIP_FETCH" = false ]; then
    ...
    if [ -f us_stock_tickers.csv ]; then
        LINES=$(tail -n +2 us_stock_tickers.csv | wc -l | tr -d ' ')
        echo "   Using attached us_stock_tickers.csv  ($LINES rows)"
    elif [ -f all_us_stocks.txt ]; then
        LINES=$(grep -c -v '^#' all_us_stocks.txt || true)
        echo "   Using existing all_us_stocks.txt  ($LINES symbols)"
    elif [ -f us_stocks.txt ]; then
